...
def enforce_report_schema(report):
    ...
    _err(f"[DEBUG-GUARD] Report top-level keys: {list(report.keys())}")
    ...
    for section, keys in FRAMEWORK_SCHEMA.items():
        ...
        if not isinstance(section_data, dict):
            _err(f"[SCHEMA] Skipping non-dict section '{section}' ({type(section_data).__name__})")
            continue
            ...
            if key not in section_data:
                ...
                if section == "footer":
                    ...
                    if key in defaults:
                        ...
                        _err(f"Auto-fix: injected missing key '{key}' in section '{section}' -> {defaults[key]}")
                        continue
    ...
    _err("[DEBUG-GUARD] Schema validation passed for all sections\n")
    return True
